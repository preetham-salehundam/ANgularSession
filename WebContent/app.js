...
app.config(function($stateProvider){
	...
	.state('smartshop.managePhoneBook',{// child state
		url:'/details/:name',
		templateUrl:'html/PhoneBook.html',
	...
	.state('Managers',{//Parent state
		url:'/managers',
		templateUrl:"html/managers.html",
	...
	.state('ProjectLead',{
		url:'/projectLeads',
		templateUrl:"html/projectLeads.html",
	...
	.state('TechLeads',{
		url:'/techleads',
		templateUrl:'html/techleads.html',
	...
	.state('SeniorSE',{
		url:'/seniorSE',
		templateUrl:'html/seniorse.html',
	...
	.state('Freshers',{
		url:'/freshers',
		templateUrl:'html/freshers.html',
...
//	return $resource('repository/:fileName');
//});
...
app.controller('phoneBookCtrl',function($scope,$state,$stateParams,$localStorage,$timeout,DetailsList){
...
$scope.saveNotes=function(contactName){ //ignore
//	if($scope.$storage && $scope.$storage[''+contactName.replace(/ /g,'')+''] ){
//		$scope.notesData=$scope.$storage[''+contactName.replace(/ /g,'')+'']['Notes_by_'+contactName.replace(/ /g,'')+''];//$scope.$storage.cheran.Notes_by_cheran
//	}
	 var tempObj={};
	 tempObj['Notes_by_'+contactName.replace(/ /g,'')+'']=$scope.notesData; // regex to remove spaces // to save obj with name of the person
	if($scope.notesData){
		$timeout(function(){
...
if($scope.$storage && $scope.$storage[''+contactName.replace(/ /g,'')+''] ){ //to prevent error when localstorage is cleared
